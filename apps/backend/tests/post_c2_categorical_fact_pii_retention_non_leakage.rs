...
#[tokio::test]
async fn c2_categorical_facts_remain_minimized_retention_classified_and_non_leaking() {
    let (_test_state, app, config, client) = test_context().await;
    let subject = sign_in(
        &app,
        "pi-user-post-c2-pii-retention",
        "post-c2-pii-retention",
    )
    .await;
    ...
    for (index, (source, mutation)) in accepted_mappings().into_iter().enumerate() {
        let idempotency_key = format!("post-c2-pii-retention-{index}");
        let snapshot = recorded(
            store
                .record_c2_bounded_promise_reliability_fact(record_input(
                    subject_account_id,
                    Some("realm-reference-post-c2-pii-retention".to_owned()),
        ...
        assert_sha256_hex(&snapshot.request_payload_hash);
    }

    assert_source_rows_are_minimized_and_retention_classified(&client, &subject_account_id).await;
    assert_mutation_rows_are_minimized_and_retention_classified(&client, &subject_account_id).await;
    assert_no_forbidden_social_trust_columns(&client).await;
    ...
    let realm = get_json(
        &app,
        &format!(
            "/api/projection/realm-trust-snapshots/realm-reference-post-c2-pii-retention/{}",
            subject.account_id
        ),
        Some(subject.token.as_str()),
    )
...
async fn assert_source_rows_are_minimized_and_retention_classified(
    ...
    let rows = client
        .query(
            ...
            ORDER BY fact_idempotency_key
            ",
            &[subject_account_id],
        )
        .await
        .expect("source metadata rows should load");
    assert_eq!(rows.len(), accepted_mappings().len());

    for row in rows {
        assert_eq!(row.get::<_, String>("evidence_posture"), "bounded");
        assert_eq!(row.get::<_, String>("reviewability_posture"), "reviewable");
        assert_eq!(
            row.get::<_, String>("retention_record_family"),
            RETENTION_RECORD_FAMILY
        );
        assert_eq!(
            row.get::<_, String>("retention_class_reference"),
            RETENTION_CLASS_REFERENCE
        );
        assert_eq!(
            row.get::<_, String>("authority_posture"),
            "writer_truth_only"
        );
        assert_sha256_hex(&row.get::<_, String>("request_payload_hash"));
    }
}

async fn assert_mutation_rows_are_minimized_and_retention_classified(
    client: &tokio_postgres::Client,
    subject_account_id: &Uuid,
) {
    let rows = client
        ...
        .await
        .expect("mutation metadata rows should load");
    assert_eq!(rows.len(), accepted_mappings().len());
    ...
    for row in rows {
        ...
        assert_sha256_hex(&row.get::<_, String>("decision_payload_hash"));
    }
}

async fn assert_no_forbidden_social_trust_columns(client: &tokio_postgres::Client) {
    let rows = client
        .query(
            "
            ...
            WHERE table_schema = 'social_trust'
              ...
              AND (
                  lower(column_name) LIKE 'raw_%'
                  OR lower(column_name) LIKE '%pi_uid%'
                  ...
                  OR lower(column_name) LIKE '%username%'
        ...
        .await
        .expect("social_trust column metadata should load");

    assert!(
        rows.is_empty(),
        "C2 categorical facts must not expose raw PII or raw evidence columns: {:?}",
        rows.iter()
            .map(|row| format!(
                "{}.{}",
