...
- `POST /api/internal/orchestration/repair` under the same internal/debug gate; runs writer-owned, forward-only orchestration recovery for stale claims, lost callback-ingest work, and verified-receipt side effects
...
Design source: ISSUE-16 recovery baseline adds an internal orchestration repair route and bounded `outbox.recovery_runs` audit rows. Repair uses the writer database only, resets expired outbox/inbox claims, closes completed-consumer/unfinished-producer gaps, re-enqueues raw callback evidence when coordination rows were lost, and applies verified receipt side effects forward once. Projection and observability rows remain evidence for operators, not repair authority. See `docs/orchestration_runtime.md`.
