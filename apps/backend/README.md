...
- `POST /api/internal/orchestration/repair` under a separate internal/debug repair gate; requires `x-musubi-operator-id` and runs writer-owned, forward-only orchestration recovery for stale claims, lost callback-ingest work, and verified-receipt side effects
...
and method surface drift, raw-string or split-prefix internal route literals,
nested internal route-prefix composition, coordination hot-table prune/delete
...
Design source: ISSUE-16 recovery baseline adds an internal orchestration repair route behind a separate repair gate and bounded `outbox.recovery_runs` audit rows. Repair uses the writer database only, resets expired outbox/inbox claims, closes completed-consumer/unfinished-producer gaps, re-enqueues raw callback evidence only when accepted writer-owned settlement evidence still matches, and applies verified receipt side effects forward once. Projection and observability rows remain evidence for operators, not repair authority. See `docs/orchestration_runtime.md`.
